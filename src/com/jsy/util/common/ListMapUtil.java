...
public class ListMapUtil {
	private static Log log = LogFactory.getLog(ListMapUtil.class);
	...
	public static List<String> splitStrToList(String idStr, String splitWord) throws Exception {
		if (CommonUtil.isEmpty(splitWord)) {
			splitWord = CommonUtil.SPLIT_COMMA;
		}
    ...
    public static String listToStringWithSperate(List<Object> list, String sperate) throws Exception {
    	...
    	if (!CommonUtil.isEmpty(list) && listLen > 0) {
     ...
     * @throws Exception
     */
    public static List<Object> getSingleListFromListMap(List<Map<String, Object>> dataList, String field) throws Exception {
     ...
     * @throws Exception
     */
    public static List<Object> getSingleListFromEntity(List<? extends Object> dataList, String field) throws Exception {
     ...
     * @throws Exception
     */
    public static List<Integer> changeListObjectToListInteger(List<? extends Object> dataList) throws Exception {
     ...
     * @throws Exception
     */
    public static List<String> changeListObjectToListStr(List<? extends Object> dataList) throws Exception {
     ...
     * @throws Exception
     */
    public static List<String> getSingleListFromListMapToString(List<Map<String, Object>> dataList, String field) throws Exception {
		Integer dataListLen = dataList.size();
    ...
    public static String arrayToString(Object[] objs,String sperate) throws Exception {
    	...
    	StringBuffer strBuf = new StringBuffer();
    	if (!CommonUtil.isEmpty(objs) && listLen > 0) {
    		for (int i = 0; i < listLen; i++) {
    ...
    public static Map<String, Object> convertEntityToMap(Object entity) throws Exception {
    	...
    	return convertEntityToMap(entity, null);
    }
    
	public static Map<String, Object> convertEntityToMap(Object entity,String timeFormat) throws Exception {
		...
		try {
				...
				if (declaredMethods[i].getName().indexOf("get") == 0) {
					...
					if (!CommonUtil.isEmpty(value)) {
						if (value instanceof Date) {
							if(StringUtil.isNull(timeFormat)){
								reMap.put(key, DateUtil.date2String((Date)value, "yyyy-MM-dd"));
							}else{
								reMap.put(key, DateUtil.date2String((Date)value, timeFormat));
							}
						} else {
							reMap.put(key, value);
						}
			...
			CommonUtil.offObject(declaredMethods, methodsLen, key, value);
		}
		return reMap;
	}
	...
	public static Object setEntityValue(Map<String, Object> srcMap, Class<?> entityClazz)
		...
		try {
			...
			CommonUtil.offObject(srcMap, dstMap, tagEntityMap, entrySet, iterator, declaredFields);
		}
		return instance;
	}
	 ...
	 * @throws Exception
	 */
	public static void setFiledVal(Field field, Object instance,
			Map<String, Object> dstMap) throws Exception {
		...
		if (dstMap.containsKey(fieldName.toLowerCase())) {
			...
			if (dstObj == null) {
				...
				case Types.BOOLEAN:
					...
					break;
				case Types.DATE:
					field.set(instance, DateUtil.str2Date(dstObj.toString()));
					break;
				default:
	 ...
	 * @throws Exception 
	 */
	public static List<Map<String, Object>> convertListEntityToListMap(List<? extends Object> listObj) throws Exception {
		
		return convertListEntityToListMap(listObj,null); 
	}
	
	public static List<Map<String, Object>> convertListEntityToListMap(List<? extends Object> listObj,String timeFormat) throws Exception {
		if (CommonUtil.isEmpty(listObj)) {
			log.debug("数据为空,将list初始化为空");
		...
		for (int i = 0; i < listObjLen; i++) {
			...
			Map<String, Object> convertEntityToMap = ListMapUtil.convertEntityToMap(object,timeFormat);
     ...
     * @param vals	需要查找的LISTMAP包含的
     * @return	返回找到的第一个包含这些值的所有MAP 
     * @throws Exception
     */
    public static List<Map<String, Object>> getListMapFromWithFixV(List<Map<String, Object>> dataList, Map<String, Object> params) throws Exception {
    	List<Map<String, Object>> reList = new ArrayList<Map<String,Object>>();
     ...
     * @throws Exception
     */
    public static Boolean isExistsInListMap(List<Map<String, Object>> dataList, Map<String, Object> params) throws Exception {
    	if (dataList == null || (dataList != null && dataList.size() < 0)) {
    		return false;
    	}
     ...
     * @return
     */
    public static void sortListMap(List<Map<String, Object>> srcList, final String sortField) throws Exception {
    	...
    	Collections.sort(srcList, new Comparator<Map<?, ?>>() {
			...
			public int compare(Map<?, ?> map1, Map<?, ?> map2) {
				...
				if (!CommonUtil.isEmpty(sortField2)) {
					num1 = Integer.parseInt(sortField2.toString());
				}
				
				if (!CommonUtil.isEmpty(sortField1)) {
    ...
    public static List<Map<String, Object>> listmapToTree(List<Map<String, Object>> srcList,
		...
		if (CommonUtil.isEmpty(srcList)) {
			log.error("源[srcList]为空,数据直接返回不进行处理");
			return srcList;
		}
		
		if (CommonUtil.isEmpty(isRank)) {
			isRank = false;
		}
		...
		List<Map<String, Object>> parentList = ListMapUtil.getListMapFromWithFixV(srcList, paramMap);
		
		if (CommonUtil.isEmpty(parentList)) {
			log.error("父级[parentList]没有任何项");
			return parentList;
		}
		
		// 为true时,排序
		if (isRank) {
			ListMapUtil.sortListMap(parentList, sortField);
		}
		...
		for (int i = 0; i < parentListLen; i++) {
			...
			Boolean exists = ListMapUtil.isExistsInListMap(srcList, paramMap1);
     ...
     * @throws Exception
     */
    public static List<Map<String, Object>> changeTimeFormatInlistMap(List<Map<String, Object>> dataList, Map<String, Object> params) throws Exception {
    	int dataListLen = dataList.size();
				...
				if (dataListMap.containsKey(key)) {
					...
					if (dataObj != null) {
						...
						if (timeStr.length() > 11) {
							Date str2Date = DateUtil.str2Date(timeStr, DateUtil.YYYY_MM_DD_HH_MM_SS);
							
							timeStr = DateUtil.date2String(str2Date, timeFormat);
    ...
    /**
     * @author: Dengpeng
     * @date: 2015年8月25日下午5:19:16
     * @description:将 key 全部转为小写
     * @param map
     * @return
     * <br>-----------------------------<br>
     * @update:
    ...
    public static Map<String, Object> mapKeyToLowerCase(Map<String, Object> map){
		...
		for (String key : keySet) {
			tempMap.put(key.toLowerCase(), map.get(key.toLowerCase()));
		}
    	return tempMap;
	}
