 ...
 */
@Service
public class CommseQuenceBusinessImpl extends BaseBusiness {

	public Map<String, Object> selectOne(Map<String, Object> dataMap) throws Exception {
		StringBuffer sql = new StringBuffer("select * from " + COMMSEQUENCE);
	...
	public Map<String, Object> selectList(Map<String, Object> dataMap) throws Exception {
		...
		List<Map<String, Object>> COMMSEQUENCEList = commonDao.selectList(sql.toString(), params);

		if (COMMSEQUENCEList != null && COMMSEQUENCEList.size() > 0) {
			reMap.put(KEY_STATE, STATE_ONE);
			reMap.put("list", COMMSEQUENCEList);
		} else {
			reMap.put(KEY_STATE, STATE_FIFTEEN);
		}
		return reMap;
	...
	public Map<String, Object> insert(Map<String, Object> dataMap) throws Exception {
		...
		Integer insertManual = commonDao.insertManual(dataMap, COMMSEQUENCE);
		if (insertManual == 1) {
			reMap.put(KEY_STATE, STATE_ONE);
		} else {
			reMap.put(KEY_STATE, STATE_FIFTEEN);
	...
	public Map<String, Object> update(Map<String, Object> dataMap) throws Exception {
		Map<String, Object> reMap = new HashMap<String, Object>();
		dataMap.put("pk", "uuid");
		Integer updateOne = commonDao.updateOne(dataMap, COMMSEQUENCE);
		if (updateOne == 1) {
			reMap.put(KEY_STATE, STATE_ONE);
		} else {
			reMap.put(KEY_STATE, STATE_FIFTEEN);
